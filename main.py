...
@app.post("/token12", response_model=TokenResponse)
async def login_for_access_token(
    form_data: Annotated[OAuth2PasswordRequestForm, Depends()]
):
    return await get_token(form_data)

@app.get("/protected", include_in_schema=True)
...
async def getCustomerDevice(
    ...
    customerId : str =Path (...,description= "A string value representing the customer id. For example:784f394c-42b6-435a-983c-b7beff2784f9"), 
    ...
    # includeCustomers : Annotated[list[str], Query()] = [True, False]    
):

    return await get_device_names(token=token, customerId=customerId, pageSize=pageSize, page=page)
